...
def build_store(force_rebuild: bool = False):
    ...
    # ✅ Embeddings ONNX (no torch)
    embedder = TextEmbedding(model_name=EMBED_MODEL)
    vecs = list(embedder.embed(all_chunks))
    doc_vecs = np.array(vecs, dtype="float32")
...
if __name__ == "__main__":
    build_store(force_rebuild=True)   # forzar rebuild porque cambia el embedding
